...
This is a shared devenv base — a reusable nix module collection that other projects import via `devenv.yaml`. Not a standalone app.
...
Nine modules in `modules/`, each exposing extensible options under `devenv-base.*` that consumer projects override in their `devenv.nix`:

- `agents-md` — generates `.pi/agent/AGENTS.md` from this file + `extraEntries`
- `ai` — MCP servers, post-edit hook
- `git-hooks` — pre-commit hooks (conventional commits, gitleaks, treefmt, shellcheck, etc.)
- `gitignore` — read-only `.gitignore` + `extraEntries`
- `languages` — nix, shell, lua enabled by default
- `lat-md` — lat.md tool
- `nvim` — `.nvim.lua` with LSPs + `extraLsps`/`extraConfig`
- `tk` — ticket tool
- `treefmt` — formatters (nixfmt, prettier, shfmt, stylua) + extensible config
...
- Tickets live in `.tickets/` directory.
